...
public:
    ...
    void startMonitoring(const Client::Pointer &aHelper) { (void)helpers.insert(aHelper); }
    void stopMonitoring(const Client::Pointer &aHelper) { (void)helpers.erase(aHelper); }
...
private:
    ...
    std::set<Client::Pointer> helpers; ///< helpers that have not been shut down yet
...
Helper::Client::Client(const char * const name): id_name(name)
{
    if (auto &monitor = Helper::MonitoringStartup())
