...
clientProcessRequest(ConnStateData *conn, const Http1::RequestParserPointer &hp, Http::Stream *context)
{
    ...
    if (!chunked && !clientIsContentLengthValid(request.getRaw())) {
        ...
        assert (repContext);
        conn->quitAfterError(request.getRaw());
        repContext->setReplyToError(ERR_INVALID_REQ, Http::scLengthRequired, nullptr, conn, request.getRaw(), nullptr, nullptr);
        assert(context->http->out.offset == 0);
        context->pullData();
        clientProcessRequestFinished(conn, request);
        return;
...
{
public:
    /// called when all required startup activities have started; TODO: Rename?
