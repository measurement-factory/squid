...
{
    fqdncache_nbgethostbyaddr(Filled(checklist)->clientAddr(), LookupDone, checklist);
}
...
ACLSourceDomainStrategy::match (ACLData<MatchType> * &data, ACLFilledChecklist *checklist)
{
    ...
    fqdn = fqdncache_gethostbyaddr(checklist->clientAddr(), FQDN_LOOKUP_IF_MISS);

    if (fqdn) {
        return data->match(fqdn);
    } else if (!checklist->sourceDomainChecked()) {
        /* FIXME: Using AclMatchedName here is not OO correct. Should find a way to the current acl */
        debugs(28, 3, "aclMatchAcl: Can't yet compare '" << AclMatchedName << "' ACL for '" << checklist->clientAddr() << "'");
