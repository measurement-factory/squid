...
public:
    ICPState(icp_common_t &aHeader, HttpRequest *, const AccessLogEntryPointer &);
    virtual ~ICPState();

    /// creates ALE (if nil), always overwriting its fields with the supplied parameters
    static void SyncAle(AccessLogEntryPointer &, const Ip::Address &caddr, const char *url, int len, int delay);
...
/// \ingroup ServerProtocolICPAPI
bool icpAccessAllowed(Ip::Address &from, HttpRequest *icp_request, const char *url, AccessLogEntryPointer &);
